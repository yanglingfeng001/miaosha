...
后期未采用thymelaeaf框架进行渲染或者缓存，改为使用前后端分离的模式，前端采用原生的HTML+jquery.js以及自己编写的js函数，后端直接返回json格式的数据，前端通过ajax异步调用的方式来实现前后端分离和数据的获取与显示。

采用页面缓存+对象缓存两种缓存策略优化用户体验

使用redis预减库存减少对mysql的访问，使用内存的标记减少对redis数据库的访问。

使用rabbitmq进行异步下单，增强用户的体验感

使用隐藏秒杀接口地址，验证码以及接口限流等三种方式来保护秒杀系统的安全不会遭到恶意访问
...
在秒杀开始之前隐藏真正的秒杀页面，防止在秒杀开始之前网站被破解（秒杀页面和秒杀地址是两个概念）